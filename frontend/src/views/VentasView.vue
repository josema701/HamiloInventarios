                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <tr v-for="item in ventas" :key="item">
                                ...
                                <td>{{ item.total - item.descuento }}</td>
                                <!-- <td>Jose Mamani</td> -->
                                <td>
                                    <button type="button" @click="verVenta(item)" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modalVer">
                                        <i class="fa fa-eye"></i>
                                    </button>
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="mb-3">
                                                    ...
                                                    <select v-model="cliente_id" id="cliente_id" class="form-select"
                                                        required>
                                                        <option value="">Seleccione cliente...</option>
                                                        <option v-for="item in clientes" :key="item"
                                                            :value="item.id">{{ item.nombres +' '+ item.apellidos }}</option>
                                                    </select>
                                                </div>
                                            </div>
                                            <!-- <div class="col-md-3">
                                            ...
                                            <table class="table">
                                                ...
                                                <tfoot>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <th colspan="2" align="left" width="10%">
                                                            <input type="number" v-model="descuento" @change="calcularTotal()" 
                                                                  class="form-control form-control-sm">
                                                        </th>
                                                    </tr>
                                                    <tr>
                                                        <th colspan="3" class="text-end">Total neto:</th>
                                                        <th colspan="2" align="left">{{ total - descuento }}</th>
                                                    </tr>
                                                </tfoot>
                                            </table>
                                        </div>
                ...
                <div class="modal-content">
                    ...
                    <div v-if="seleccionado && seleccionado.id" class="modal-body">
                        <table class="table table-sm">
                            ...
                            <tr>
                                <th>Descuento</th>
                                <td>{{ seleccionado.descuento }}</td>
                            </tr>
                            <tr>
                                <th>Total neto</th>
                                <td>{{ seleccionado.total - seleccionado.descuento }}</td>
                            </tr>
                            <tr>
                                <th>Fecha</th>
...
const obtenerDatos = async (param = 'p0') => {
    try {
        let url = baseURL + 'ventas';
        if (param != 'p0') {
            url = baseURL + 'ventas?page=' + param;
        }
...
const obtenerClientes = async () => {
    try {
        const { data } = await axios.get(baseURL + 'clientes', header);
        console.log(data);
...
const obtenerProductos = async () => {
    try {
        const { data } = await axios.get(baseURL + 'productos/activos', header);
        console.log(data);
...
const actualizarCantidad = (i, cant) => {
    ...
    if(det.stock >= cant){
        ...
        det.cantidad = det.stock;
        alert('No hay suficiente stock, el stock existente es: ' + det.stock);
    }
...
const registrar = async () => {
    ...
    if(cliente_id.value == '' || total.value == '' || descuento.value < 0 || fecha.value == '' || detalles.value.length == 0 ) {
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
    ...
    try {
        const { data } = await axios.post(baseURL + 'ventas/store', datos, header);
        console.log(data);
        reset();
        // document.querySelector('button.btn-close').click();
        var myModalEl = document.getElementById('modalVentas');
...
const verVenta = async(item) => {
    seleccionado.value = item;

    // obtener el detalle
    try {
        const { data } = await axios.get(baseURL + 'ventas/detalle/' + item.id, header);
        console.log(data);
...
const eliminar = async(item) => {
    if(confirm('¿Deseas eliminar la venta ' + item.comprobante + '?')) {
        try {
            const { data } = await axios.delete(baseURL + 'ventas/eliminar/' + item.id, header);
            console.log(data);
            obtenerDatos();
            obtenerProductos();
        } catch (error) {
